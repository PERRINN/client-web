      ...
      <div style="padding:10px">
        <div style="clear:both">
          <div style="float:left;width:200px">
            ...
            <span class="buttonBlack" *ngIf="focusUserLastMessageObj?.contract?.createdTimestamp&&!focusUserLastMessageObj?.contract?.signed&&UI.currentUser=='QYm5NATKa6MGD87UpNZCTl6IolX2'" style="margin:15px;font-size:10px" (click)=signContract()>Sign contract</span>
          </div>
        ...
        <div class="buttonWhite" *ngIf="UI.currentUser!=focusUserLastMessageObj?.user" (click)="newMessageToUser()" style="float:left;font-size:10px;padding:2px 4px 2px 4px;margin-right:5px">New message to {{focusUserLastMessageObj?.name}}</div>
      </div>
      <div class="seperator" style="width:100%;margin:0px"></div>
    </div>
    <div *ngIf="scope=='all'">
        ...
        <li *ngFor="let message of messages|async;let first=first;let last=last"
          ...
          <div *ngIf="scope!='all'&&(mode=='30days'||mode=='24months'||mode=='chain')">
            ...
            <div class="tableRow">
              ...
              <div style="float:left;text-align:center;width:75px;height:20px;border-style:solid;border-width:0 1px 1px 0">{{UI.formatSharesToCurrency(null,message.payload.doc.data()?.wallet?.shareBalance)}}</div>
              <div style="float:left;text-align:center;width:65px;height:20px;border-style:solid;border-width:0 1px 1px 0;font-size:10px">{{first?'':UI.formatSharesToCurrency(null,message.payload.doc.data()?.wallet?.shareBalance-previousBalance)}}</div>
