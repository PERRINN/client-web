      ...
      <div *ngIf="!showChatDetails">
        <div style="float:left;width:80%;margin:0 5px 0 10px;min-height:40px">
          ...
          <div *ngIf="math.floor(eventDate/60000-UI.nowSeconds/60)>-60" style="clear:both">
            ...
            <div *ngIf="math.floor(eventDate/60000-UI.nowSeconds/60)<=0&&math.floor(eventDate/60000-UI.nowSeconds/60)>-60" style="float:left;background-color:darkred;color:whitesmoke;padding:0 5px 0 5px">Now</div>
          ...
          <div *ngIf="fund?.active" style="clear:both">
            ...
            <div style="float:left;height:20px;background-color:darkred;margin-left:-65px" [style.width]="(fund?.amountGBPRaised/fund?.amountGBPTarget)*65+'px'"></div>
          ...
          <div *ngIf="(UI.nowSeconds<survey?.expiryTimestamp/1000)&&survey?.createdTimestamp" style="clear:both">
            ...
            <div *ngIf="!chatLastMessageObj?.recipients[UI.currentUser]?.voteIndexPlusOne" style="clear:both;color:darkred;margin:0 5px 0 5px">vote now</div>
          </div>
        </div>
  ...
  <div class="sheet" *ngIf="showChatDetails" style="padding-top:40px">
    ...
    <ul class="listLight" style="margin:10px">
      <li *ngFor="let recipient of chatLastMessageObj?.recipientList" style="float:left">
        ...
        <div style="float:left;cursor:pointer;margin:10px 15px 3px 3px;font-size:12px;line-height:10px;font-family:sans-serif;color:darkred" (click)="removeRecipient(recipient,chatLastMessageObj?.recipients[recipient]?.name)">X</div>
      </li>
    </ul>
    ...
    <div class="seperator" style="width:100%;margin:0px"></div>
    <span style="margin:10px">Sending {{UI.appSettingsPayment.currencyList[UI.currentUserLastMessageObj.userCurrency].designation}}:</span>
    ...
    <div>
      ...
      <div style="font-size:12px;margin:10px">{{eventDate==0?'':eventDate|date:'EEEE d MMM HH:mm'}}</div>
      <div class="buttonWhite" *ngIf="eventDate!=chatLastMessageObj?.eventDate||eventDescription!=chatLastMessageObj?.eventDescription" style="clear:both;width:100px;font-size:10px;margin:10px" (click)="saveEvent()">Save event</div>
    ...
    <div class="seperator" style="width:100%;margin:0px"></div>
    <div>
      <div *ngIf="survey.createdTimestamp" style="font-size:12px;margin:10px">created on {{survey.createdTimestamp|date:'EEEE d MMM HH:mm'}} expiring on {{survey.expiryTimestamp|date:'EEEE d MMM HH:mm'}}</div>
    ...
    <div>
      <ul style="list-style:none;">
        <li *ngFor="let message of messages|async;let first=first;let last=last;let i=index">
          ...
          <div [style.background-color]="(message.payload?.user==UI.currentUser)?'darkslategray':'black'"
            ...
            <div style="cursor:pointer;clear:both;height:15px" (click)="messageShowActions.includes(message.key)?messageShowActions.splice(messageShowActions.indexOf(message.key),1):messageShowActions.push(message.key)">
              ...
              <span *ngIf="message.payload?.contract?.hoursValidated>0" style="float:right;font-size:10px;margin:0 5px 2px 0;line-height:15px">+{{UI.formatSharesToCurrency(null,message.payload?.contract?.amount)}} earned ({{UI.formatSecondsToDhm1(message.payload?.contract?.hoursValidated*3600)}}declared in {{UI.formatSecondsToDhm1(message.payload?.contract?.hoursAvailable*3600)}} window)</span>
              <span *ngIf="message.payload?.purchaseCOIN?.amount>0" style="float:right;font-size:10px;margin:0 5px 2px 0;line-height:15px">+{{UI.formatSharesToCurrency(null,message.payload?.purchaseCOIN?.amount)}} purchased</span>
              <span *ngIf="message.payload?.transactionIn?.amount>0" style="float:right;font-size:10px;margin:0 5px 2px 0;line-height:15px">+{{UI.formatSharesToCurrency(null,message.payload?.transactionIn?.amount)}} received</span>
  ...
  <div class="sheet" *ngIf="!showImageGallery">
    <div class="fixed" style="bottom:0">
      ...
      <div style="clear:both;float:left;width:90%">
        <input autocapitalize="none" style="float:left;padding:9px;resize:none;overflow-y:scroll"  [style.width]="imageDownloadUrl?'80%':'95%'" maxlength="500" (keyup.enter)="addMessage()" [(ngModel)]="draftMessage" placeholder="Reply all">
      ...
      <div *ngIf="!draftMessage&&!imageDownloadUrl" style="float:right;width:10%;cursor:pointer">
        ...
        <span class="material-icons-outlined" style="margin:15px 5px 5px 5px;font-size:30px;color:gainsboro">photo_camera</span>
        </label>
      </div>
    </div>
  </div>
