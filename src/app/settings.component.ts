...
@Component({
  ...
  <span *ngIf="UI.currentUserLastMessageObj?.userStatus?.isContributor" style="font-size:10px"> Contributor ({{UI.currentUserLastMessageObj?.contract?.position}} Level {{UI.currentUserLastMessageObj?.contract?.level}})</span>
  ...
  <div class='sheet'>
    ...
    <div class="seperator" style="width:100%;margin:0px"></div>
      ...
      <div *ngIf="UI.currentUserLastMessageObj?.contract?.createdTimestamp&&UI.currentUserLastMessageObj?.contract?.signed" style="float:left;margin:15px;font-size:10px;color:midnightblue">Signature valid for level {{UI.currentUserLastMessageObj?.contract?.signedLevel}}, you will receive {{UI.currentUserLastMessageObj?.contract?.hourlyRate}} COINS per hour when you post messages in your log.</div>
