...
@Component({
  ...
  <div class='sheet'>
    ...
    <div class="seperator" style="width:100%;margin:0px"></div>
      ...
      <div *ngIf="UI.currentUserLastMessageObj?.contract?.createdTimestamp&&UI.currentUserLastMessageObj?.contract?.signed" style="float:left;margin:15px;font-size:10px;color:midnightblue">Signature valid for level {{UI.currentUserLastMessageObj?.contract?.signedLevel}}, you will receive {{UI.currentUserLastMessageObj?.contract?.hourlyRate}} COINS per hour when you delare working hours.</div>
